...
def main():
    ...
    if missing_values_count != 0:
        if 'ma_0_model.pkl' in ma_model_name:
            values['Data'].interpolate(method='linear', inplace=True)
        if 'ma_mean_model.pkl' in ma_model_name:
            values['Data'].fillna(values['Data'].mean(), inplace=True)
        if 'ma_forward_model.pkl' in ma_model_name:
            values['Data'].fillna(method='ffill', inplace=True)
        if 'ma_backward_model.pkl' in ma_model_name:
    ...
    for i in range(n_periods):
        ...
        new_average = df['Data'].iloc[-window_size:].mean()

        # Thêm giá trị dự đoán vào DataFrame
        df = df.append({'Time': df['Time'].iloc[-1] + 1, 'Data': None,
                       'Moving_Average': new_average}, ignore_index=True)

    print("Predictions: ", df['Moving_Average'].tail(n_periods))
